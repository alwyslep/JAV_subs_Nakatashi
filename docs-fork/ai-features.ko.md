...
> **위 13종은 프롬프트를 직접 편집할 수 있습니다.** 자동 번역 창의 **설정** 버튼 →
> **프롬프트** 입력란(`TranslateSettingsWindow`). 엔진마다 따로 저장되고, `{0}`(원본 언어)과
> `{1}`(대상 언어)이 들어 있는지 저장할 때 검사합니다. **기본값으로 초기화** 버튼으로 언제든
> 되돌릴 수 있습니다.
>
> 등장인물 관계나 화계를 지정하고 싶다면 여기가 그 자리입니다 —
> 자세한 내용은 [화계 기능 분석](ai-register-proposal.ko.md) 참조.
> 전용 번역기 12종은 프롬프트 개념이 없어 이 입력란이 숨겨집니다.
