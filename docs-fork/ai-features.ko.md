...
> 대상 버전: **v5.1.0-rc18** (`93f828633`) 기준, JAV_subs_Nakatashi 포크
> 작성일: 2026-07-27 · 개정 2026-07-30(포크가 추가한 두 패스 반영)
> 메뉴 경로는 이 포크의 [Phase 4 메뉴 재편](../CLAUDE.md) 기준입니다.

앱 안에서 **AI가 실제로 개입하는 기능은 10가지**입니다 — upstream의 8가지와
**이 포크가 추가한 2가지**(화계 맞추기·이름 검사). 대부분 로컬 모델과
...
- [9. 화계 맞추기 ★포크](#9-화계-맞추기-이-포크가-추가)
- [10. 이름 검사 ★포크](#10-이름-검사-이-포크가-추가)
...
| **9** | **화계 맞추기** ★포크 | 그리드 우클릭 / AI 도우미 5번째 버튼 | 선택한 줄의 **어미**를 한 화계로 통일 | 3 |
| **10** | **이름 검사** ★포크 | 도구(T) > 이름 검사... | 한 인물을 두 표기로 적은 곳을 찾고, 고른 표기를 시리즈에 기억 | 3 |

> 일괄 변환(**도구 > 일괄 변환**)에서도 **자동 번역**을 처리 단계 중 하나로 쓸 수 있습니다.
>
> 9·10번은 엔진 설정(엔진·URL·모델·API 키)을 **1번 AI 검토와 공유**합니다 — 어느 창에서 바꿔도
> 같습니다. 사용자용 설명은 [`docs/features/speech-register.md`](../docs/features/speech-register.md),
> [`docs/features/name-check.md`](../docs/features/name-check.md), 설계 근거와 실측은
> [영상 메타데이터 계획서](video-metadata-plan.ko.md)에 있습니다.
...
**그리드 우클릭 > 화계 맞추기...** (또는 텍스트 상자 우클릭 / AI 도우미 창의 5번째 버튼)

선택한 줄의 **어미**를 한 화계(하십시오체·해요체·해체·해라체)로 통일합니다. 낱말은 건드리지
않습니다. **메뉴에 없는 것은 의도**입니다 — 화계는 "누가 누구에게"의 성질이고 한 작품에 여러
관계가 동시에 있으므로, 파일 전체 패스는 그것을 뭉개 버립니다. 그래서 선택 기반이고, 진입점도
이미 여러 줄을 고른 우클릭 시점입니다.

관계 메모(`{notes}`)는 **가이드북 → 영상 태그 → 공용 카탈로그** 순으로 **자동 채워지며**
(추가 LLM 호출 0회) 창이 출처를 표시합니다. 사용자가 고쳐 저장한 것만 **품번 단위로** 기록되어
다음 번역 런이 씁니다. 어미 말고 다른 곳이 바뀐 제안은 경고로 표시됩니다.

---

## 10. 이름 검사 ★이 포크가 추가

**도구(T) > 이름 검사...** — AI 검토 바로 아래입니다(둘 다 파일 전체를 한 번에 읽습니다).

한 인물을 자막이 두 가지로 적었거나 이름을 **뜻으로 옮긴** 곳(`鈴の家りん` → "린의 집 인")을
찾습니다. 다른 두 패스와 달리 **교정된 줄이 아니라 이름 표를 받고**, 치환은 편집기가 직접
합니다 — 고정에는 `(원문 → 표기)` 짝이 필요하고, 그 짝은 교정된 줄에 없습니다. 수락한 표기는
공용 용어집에 `pinned`로 들어가 번역기의 재수확·정제가 덮지 못합니다.

**실측(8편):** 5편은 발견 0건, 고정 가능은 4건 중 2건. 고정률은 모델 성능이 아니라 **원문
표기를 알 수 있는지**(=영상 태그 보유율)에 달려 있습니다. 자세한 수치와 설계 근거는
[영상 메타데이터 계획서의 단계 6](video-metadata-plan.ko.md)에 있습니다.
